<template>
  <div>
    <h2 id="page-heading" data-cy="LabelHeading">
      <span v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.home.title')" id="label-heading">Labels</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link
          :to="{ name: 'LabelCreate' }"
          tag="button"
        ...
        >
          ...
          <span v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.home.createLabel')"> Create a new Label </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && labels && labels.length === 0">
      <span v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.home.notFound')">No labels found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('labelName')">
              <span v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.labelName')">Label Name</span>
        ...
        <tbody>
          <tr v-for="label in labels" :key="label.id" data-cy="entityTable">
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link
                  :to="{ name: 'LabelView', params: { labelId: label.id } }"
                  tag="button"
                ...
                <router-link
                  :to="{ name: 'LabelEdit', params: { labelId: label.id } }"
                  tag="button"
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div class="modal-body">
        <p id="jhi-delete-label-heading" v-text="$t('jhipsterSampleApplicationVueApp.testRootLabel.delete.question', { id: removeId })">
