<template>
  <div class="modal-body">
    <span class="badge bg-primary" @click="threadDumpFilter = ''"
      >{{ t$('metrics.jvm.threads.all') }}&nbsp;<span class="badge rounded-pill bg-default">{{ threadDumpData.threadDumpAll }}</span></span
    >&nbsp;
    <span class="badge bg-success" @click="threadDumpFilter = 'RUNNABLE'"
      >{{ t$('metrics.jvm.threads.runnable') }}&nbsp;<span class="badge rounded-pill bg-default">{{
        threadDumpData.threadDumpRunnable
      }}</span></span
    >&nbsp;
    <span class="badge bg-info" @click="threadDumpFilter = 'WAITING'"
      >{{ t$('metrics.jvm.threads.waiting') }}&nbsp;<span class="badge rounded-pill bg-default">{{
        threadDumpData.threadDumpWaiting
      }}</span></span
    >&nbsp;
    <span class="badge bg-warning" @click="threadDumpFilter = 'TIMED_WAITING'"
      >{{ t$('metrics.jvm.threads.timedwaiting') }}&nbsp;<span class="badge rounded-pill bg-default">{{
        threadDumpData.threadDumpTimedWaiting
      }}</span></span
    >&nbsp;
    <span class="badge bg-danger" @click="threadDumpFilter = 'BLOCKED'"
      >{{ t$('metrics.jvm.threads.blocked') }}&nbsp;<span class="badge rounded-pill bg-default">{{
        threadDumpData.threadDumpBlocked
      }}</span></span
    >&nbsp;
    <div class="mt-2">&nbsp;</div>
    Filter
